...
Повторный запуск идемпотентен для уже установленного `hysteria2`: если IP/порт/TLS совпадают, скрипт проверит существующий контейнер и выведет текущую ссылку без пересоздания. Если установка оборвалась до конца, сначала выполните `uninstall.sh --remove-files --remove-cert --ip SERVER_IP --force`, затем запустите clean install заново.
